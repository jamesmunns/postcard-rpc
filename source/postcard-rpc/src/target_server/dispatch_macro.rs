...
macro_rules! define_dispatch {
    ...
    (@arm blocking ($endpoint:ty) $handler:ident $context:ident $header:ident $req:ident $sender:ident) => {
        {
            $crate::standard_icd::Outcome::Reply($handler($context, $header.clone(), $req))
        }
    };
    // This is the "async execution" arm for defining an endpoint
    (@arm async ($endpoint:ty) $handler:ident $context:ident $header:ident $req:ident $sender:ident) => {
        {
            $crate::standard_icd::Outcome::Reply($handler($context, $header.clone(), $req).await)
        }
    };
                ...
                match hdr.key {
                    $(
                        <$endpoint as $crate::Endpoint>::REQ_KEY => {
                            ...
                            let resp: Outcome<<$endpoint as $crate::Endpoint>::Response> = define_dispatch!(@arm $flavor ($endpoint) $handler context hdr req sender);
                            match resp {
                                Outcome::Reply(t) => {
...
pub mod fake {
    ...
    async fn test_alpha_handler(_context: &mut TestContext, _header: WireHeader, _body: AReq) -> AResp {
        todo!()
    }

    async fn test_beta_handler(_context: &mut TestContext, _header: WireHeader, _body: BReq) -> BResp {
        todo!()
    }

    async fn test_gamma_handler(_context: &mut TestContext, _header: WireHeader, _body: GReq) -> GResp {
        todo!()
    }
    ...
    // #[embassy_executor::task]
    // async fn test_epsilon_handler_task(_context: &mut TestContext, _header: WireHeader, _body: EReq, _sender: Sender<FakeMutex, FakeDriver>) {
    //     todo!()
    // }
}
