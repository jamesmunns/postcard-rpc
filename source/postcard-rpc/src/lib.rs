...
pub trait Endpoint {
    ...
    /// The unique [Key] identifying this Endpoint (path + request + response schemas)
    const ENDPOINT_KEY: Key;
    /// The unique [Key4] identifying this Endpoint
    const ENDPOINT_KEY4: Key4 = Key4::from_key8(Self::ENDPOINT_KEY);
    /// The unique [Key2] identifying this Endpoint
    const ENDPOINT_KEY2: Key2 = Key2::from_key8(Self::ENDPOINT_KEY);
    /// The unique [Key1] identifying this Endpoint
    const ENDPOINT_KEY1: Key1 = Key1::from_key8(Self::ENDPOINT_KEY);
}
...
pub struct DeviceMap {
    ...
    /// The list of endpoints by path string and endpoint key
    pub endpoints: &'static [(&'static str, Key)],
...
/// endpoints by path and endpoint key
#[derive(Debug)]
