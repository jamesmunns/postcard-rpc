...
mod test {
    ...
    fn min_test_1() {
        const MINA: usize = min_key_needed(&[&[
            Key::from_bytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
            Key::from_bytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01]),
        ]]);
        assert_eq!(1, MINA);
    ...
    fn min_test_2() {
        const MINA: usize = min_key_needed(&[&[
            Key::from_bytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
            Key::from_bytes([0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01]),
        ]]);
        assert_eq!(2, MINA);
        const MINB: usize = min_key_needed(&[
            &[Key::from_bytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])],
    ...
    fn min_test_4() {
        const MINA: usize = min_key_needed(&[&[
            Key::from_bytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
            Key::from_bytes([0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01]),
        ]]);
        assert_eq!(4, MINA);
        const MINB: usize = min_key_needed(&[
            &[Key::from_bytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])],
    ...
    fn min_test_8() {
        const MINA: usize = min_key_needed(&[&[
            Key::from_bytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
            Key::from_bytes([0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01]),
        ]]);
        assert_eq!(8, MINA);
        const MINB: usize = min_key_needed(&[
            &[Key::from_bytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])],
